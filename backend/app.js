...
app.get("/api/blogs/:id", (req, res) => {
  // get follow id
  const blogID = blog.find((b) => {
    return b.id === parseInt(req.params.id);
  });
  //   if cant find
  if (!blogID) {
    //404
    return res.status(404).send("the blog with given ID was not found");
  } else {
    res.send(blogID);
  }
});
...
// connect to database
const uri =
  "mongodb+srv://quan:123@realmcluster.psqyg.mongodb.net/blogDatabase?retryWrites=true&w=majority";
